...
### iPadOS Safari ten onrechte als Mac Safari gedetecteerd (2026-05-15 vervolg 2)
**Probleem:** Lex testte de banner op iPad in Safari en zag niets — geen 📲-knop in de header en geen banner. Op iPhone Safari werkte alles wel, en in de Chromium-preview ook. Diagnose: moderne iPadOS Safari rapporteert zich standaard met user-agent string die "Macintosh" bevat (sinds iPadOS 13, met "Vraag om bureaubladsite" als default voor grote iPads). Mijn `isMacSafari`-uitsluiting (om de banner niet te tonen op Mac Safari waar PWA-installatie niet kan) ving daardoor iPad-gebruikers per ongeluk ook. Hetzelfde gold voor `isIosSafari`: de check op "iPad" in de UA-string faalde, dus de install-knop werd niet geïnitialiseerd.
**Opgelost:** onderscheid Mac van iPad via `navigator.maxTouchPoints` (Mac trackpad: 0-1, iPad: 5+). `isIosSafari` herkent nu ook iPadOS desktop-mode (Macintosh-UA + touch>1). `shouldShowInstallBanner` maakt isMacSafari alleen true bij touch≤1. Vijf test-scenario's geverifieerd in preview-server. SW-cache v5 → v6.
**Geleerd:** user-agent-detectie als enige bron is fragiel — Apple's "Vraag om bureaubladsite" maskeert iPads als Macs. Combineren met feature-detectie (`navigator.maxTouchPoints`, `'ontouchstart' in window`) maakt platform-keuzes robuuster. Bevestigt het bredere principe: ontwerp- en testdoorloop op het beoogde eindgebruiker-apparaat onthult bugs die preview-servers (Chromium desktop) niet vinden.
...
- [ ] **In v0.3: installeer-banner-iteratie opnemen** als sub-cyclus in H11 (Cyclus 5) — details in `docs/onderzoek/CHANGELOG.md` onder "Te verwerken in v0.3"
...
- **2026-05-15 (vervolg 2)** — Bug-fix: iPadOS Safari werd ten onrechte als Mac Safari gedetecteerd (moderne iPadOS UA bevat "Macintosh"). Lex testte op iPad in Safari, zag niets. Fix: onderscheid Mac/iPad via `navigator.maxTouchPoints`. `isIosSafari` herkent nu ook iPadOS desktop-mode; `isMacSafari` checkt op touch≤1. SW-cache v5 → v6. Op iPad bevestigd door Lex (installatie via iOS-stappen geslaagd). Methodische opmerking voor rapport v0.3: zelf-test op beoogd eindapparaat onthulde bug die preview-server (Chromium) niet vond — past in DBR-cyclus 5.
